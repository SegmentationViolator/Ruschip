...
impl Backend {
    pub(super) fn execute(
        ...
        match instruction.operator_code() {
            ...
            0x0 if instruction.operand_nnn() == 0x0FF => self.display_buffer.half_resolution = false,
            ...
            0xD => {
                ...
                } else { colliding_rows as u8 }
            }

            0xF if instruction.operand_nn() == 0x29 => {
                ...
                if character_code >= super::KEY_COUNT {
                    if character_code & 0x10 != 0 && character_code & 0xF < HIRES_CHARACTER_COUNT {
                        self.inner.registers.address = chip8::FONT_SIZE + (character_code & 0xF) * HIRES_CHARACTER_SIZE
                    }
